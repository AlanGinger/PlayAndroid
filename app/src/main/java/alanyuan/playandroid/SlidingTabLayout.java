...
//    }
    ...
    public void setData(List<String> tabs) {
        mTabs.clear();
        mTabs.addAll(tabs);
        notifyDataSetChanged();
    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            CharSequence title = mTabs.get(i);
                ...
                if (mCurrentPosition != position) {
                    ...
                    animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                        ...
                        public void onAnimationUpdate(ValueAnimator animation) {
                            ...
                            mCurrentPositionOffset = (float) curValue / 1000;
...
//                            unSelect(tab);
    ...
    public Pair<Float, Float> getIndicatorCoordinates() {
        ...
        View currentTextView = ((ViewGroup) mTabsContainer.getChildAt(mCurrentPosition)).getChildAt(0);
        float lineLeft = currentTab.getLeft() + currentTextView.getLeft();
        float lineRight = currentTab.getLeft() + currentTextView.getRight();
        // if there is an offset, start interpolating left and right coordinates between current and next tab
        if (mCurrentPositionOffset > 0f && mCurrentPosition < mTabCount - 1) {
            View nextTab = mTabsContainer.getChildAt(mNextPosition);
            View nextTextView = ((ViewGroup) mTabsContainer.getChildAt(mNextPosition)).getChildAt(0);
            final float nextTabLeft = nextTab.getLeft() + nextTextView.getLeft();
            final float nextTabRight = nextTab.getLeft() + nextTextView.getRight();
            lineLeft = (mCurrentPositionOffset * nextTabLeft + (1f - mCurrentPositionOffset) * lineLeft);
            lineRight = (mCurrentPositionOffset * 2 * nextTabRight + (1f - mCurrentPositionOffset) * lineRight);
            if (lineRight >= nextTabRight) {
